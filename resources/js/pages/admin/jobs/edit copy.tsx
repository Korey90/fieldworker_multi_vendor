...
import { type BreadcrumbItem, type Job, type Location, type User, type Worker } from '@/types';
import AssignedWorkersSelect from '@/components/AssignedWorkersSelect';
...
interface JobFormData {
    ...
    description: string;
    location_id: string;
    assigned_user_id: string; // zostawisz dla backward compatibility, ale nie będzie używany
    assigned_user_ids: string[]; // <-- NOWE
...
export default function JobEdit({ job, locations, workers }: JobEditProps) {
...
const { data, setData, put, processing, errors, delete: destroy } = useForm<JobFormData>({
    ...
    assigned_user_id: job.assigned_user_id?.toString() || '', // legacy
    assigned_user_ids: job.workers?.map((w: any) => w.id.toString()) || [], // jeśli job ma przypisanych
...
console.log('Initial assigned_user_ids:', data.assigned_user_ids);
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-6">
                            ...
                            <AssignedWorkersSelect workers={workers} data={data} setData={setData} />
                            
                            {/* Info about assignment changes */}
                            <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                                <p className="text-sm text-blue-800">
                                    <strong>Uwaga:</strong> Zmiany w przypisaniach pracowników będą zastosowane po kliknięciu "Save Changes". 
                                    Tylko różnice zostaną zaktualizowane - istniejące przypisania zostaną zachowane jeśli pracownik pozostaje na liście.
                                </p>
                            </div>
                        </CardContent>
                    </Card>
                                ...
                                <div className="space-y-3">
                                    {job.assignments.map((assignment) => (
                                        <div key={assignment.id} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                                            <div>
                                                <p className="font-medium">{assignment.worker?.first_name} {assignment.worker?.last_name}</p>
