...
const WorkersEdit: React.FC<Props> = ({ worker, locations, skills, roles }) => {
    ...
    // Initialize selected skills and roles on component mount
    useEffect(() => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        put(route('tenant.workers.update', worker.id));
    };

    const handleSkillToggle = (skillId: string, checked: boolean) => {
        if (checked) {
            // Find existing skill level or default to beginner
            const existingSkill = worker.skills.find(s => s.id === skillId);
            const level = existingSkill?.pivot?.level || 'beginner';
            setSelectedSkills(prev => [...prev, { id: skillId, level }]);
        } else {
    ...
    return (
        <AppLayout>
            <Head title={`Edytuj pracownika: ${getWorkerFullName()}`} />
                    ...
                    <div className="mb-6">
                        <div className="flex items-center gap-4 mb-4">
                            <Button
                                variant="outline"
                                size="sm"
                                onClick={() => router.visit(route('tenant.workers.show', worker.id))}
                            >
                        ...
                        <h1 className="text-3xl font-bold text-gray-900">
                            Edytuj pracownika: {getWorkerFullName()}
                        </h1>
